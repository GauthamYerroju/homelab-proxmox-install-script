...
function start_script() {
  CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" \
    --title "SETTINGS" \
    --menu "Choose configuration type:" 15 60 3 \
    "1" "Default Settings (safe for most hardware setups)" \
    "2" "Optimized Docker VM Settings (for heavier Docker usage with HBA and NIC passthrough)" \
    "3" "Advanced Settings" 3>&1 1>&2 2>&3)
  ...
  case "$CHOICE" in
    ...
    2)
      header_info
      echo -e "${DEFAULT}${BOLD}${BL}Using Optimized Docker VM Settings${CL}"
      optimized_settings
      ;;
    3)
      header_info
...
msg_info "Installing OpenSSH server and setting up root login"
virt-customize -q -a "${FILE}" --install openssh-server,jq >/dev/null 2>&1
virt-customize -q -a "${FILE}" --run-command "mkdir -p /etc/ssh/sshd_config.d"
virt-customize -q -a "${FILE}" --upload /dev/stdin:/etc/ssh/sshd_config.d/99-root.conf <<'EOF'
PermitRootLogin yes
PasswordAuthentication yes
EOF
virt-customize -q -a "${FILE}" --root-password password:docker
virt-customize -q -a "${FILE}" --run-command "systemctl enable ssh" >/dev/null 2>&1
msg_ok "OpenSSH installed and enabled"
...
msg_ok "Completed Successfully! Default are root:docker\n"
